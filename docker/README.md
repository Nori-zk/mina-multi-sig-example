...
- Fill in `docker/.env.notification-server` with the values from `npm run frost-notification-config` (run from the project root). The script generates all values except `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` which you fill in manually. The file contains:
  - `NOTIFICATION_NAMESPACE` — must match the `NOTIFICATION_NAMESPACE` in each participant's `.env` (default: `nori-multisig`)
  - `NOTIFICATION_ALLOWED_KEYS` — comma-separated X25519 public keys of all committee members (generated by the script)
  - `NOTIFICATION_SERVER_PRIVKEY` — the server's X25519 private key for DH-HMAC JWT auth (generated by the script)
  - `TELEGRAM_BOT_TOKEN` — from [@BotFather](https://t.me/botfather)
  - `TELEGRAM_CHAT_ID` — your committee's Telegram group chat ID
