...
**What to do next:** Wait until all members have shared and imported each other's contacts before proceeding.
...
Then share these values with all committee members — they need to add them to their `.env` files too.
...
**What to do next:** Once DKG completes, the script displays the admin group public key. Note it down — this becomes the admin contract address. All participants should verify their group public key matches.
...
Same process. Note the token group public key — this becomes the token contract address.
...
1. Your `.env` has `MINA_RPC_NETWORK_URL`, `MINA_NETWORK`, and `MINA_SENDER_PRIVATE_KEY` set
2. `MINA_SENDER_PRIVATE_KEY` is the fee payer account — it must have enough MINA to cover transaction fees and new account creation fees
...
2. **Compilation and proving** — compiles both contracts and proves the deploy + initialize transaction. This takes several minutes due to ZK proof generation.
...
5. **Admin group signing** — starts a FROST coordinator session for the admin group. The script blocks here, polling frostd every 2 seconds, waiting for participants to verify and sign. This may take a long time — participants need to clone the code, build it, inspect it, and join.
6. **Token group signing** — same process for the token group. Blocks again until all participants sign.
7. **Submission** — re-signs the transaction with the fee payer key and submits to the Mina network. Waits for inclusion in a block.
...
4. **Proving** — builds and proves the `updateVerificationKey` transaction using the current circuit with the target VK data. Several minutes.
...
7. **Admin group signing** — blocks polling frostd until all participants verify and sign.
8. **Submission** — re-signs with fee payer and submits. Waits for block inclusion.
