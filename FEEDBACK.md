...
We observe intermittent `SnowError(Decrypt)` failures during dual-session signing ceremonies that we have not been able to root-cause definitively. The following are areas in frostd and the client that may contribute. None are confirmed as the cause but all are worth investigating.

1. **Separate locks on `sessions` and `sessions_by_pubkey`** — `close_session` in `frostd/src/functions.rs:292-293` acquires write locks on these two maps in sequence, not atomically. A concurrent `list_sessions` read on `sessions_by_pubkey` could see state between the two lock acquisitions. Similarly `create_new_session` (`functions.rs:115-116`) acquires both locks sequentially.

2. **Non-deterministic recipient ordering** — The coordinator iterates `self.pubkeys.keys()` (`src/coordinator/comms/http.rs:181`) which is a `HashMap` with no guaranteed order. The coordinator sends all chunks to recipient A, then all chunks to recipient B. If the order varies between runs, the timing of when each participant receives their chunks changes, which may interact with the `receive` drain behaviour.

3. **`drain(..)` returns partial chunk sets** — `receive` in `frostd/src/functions.rs:271` drains the entire queue at once. If the coordinator has sent 1 of 3 chunks when the participant polls, the participant gets 1 message. The participant's chunking logic expects a header followed by N chunks. A partial read could leave the participant's Noise state mid-stream, and subsequent messages arrive on the next poll with potentially misaligned state.

4. **Access tokens accumulate** — The `sessions` CLI command (`src/cli/session.rs`) logs in but never logs out. Each `ensure_clean_sessions` call spawns a new process that logs in with the same pubkey, creating a new access token. Participant retry logic for the token group session can spawn multiple processes in quick succession, each logging in. frostd maps multiple access tokens to the same pubkey simultaneously. The interaction of multiple concurrent tokens for the same identity is untested.
